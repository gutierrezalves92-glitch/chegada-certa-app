...
async function exportArrivalsCsv() {
  ...
  downloadCsv(`chegada-certa_chegadas_${todayLocalDateStr()}.csv`, [
    ...
    { label: 'Malotes coletados', value: (a) => (a.collected_bags ?? '') },
...
async function openRoute(id) {
  ...
  const stopsHtml = (r.stops && r.stops.length)
    ? `<h4>Roteiro planejado</h4><p class="muted">HUB PRINCIPAL → ${r.stops.map((s) => s.base).join(' → ')}</p>`
    : '';
