...
async function startJourney(body) {
  const route = await db.get('SELECT * FROM routes WHERE id = ?', [body.route_id]);
  if (!route) {
    const err = new Error('Rota não encontrada — pode ter sido removida ou o link/placa está incorreto.');
    err.statusCode = 404;
    throw err;
  }
  if (body.plate && String(body.plate).toUpperCase() !== String(route.plate).toUpperCase()) {
    const err = new Error('A placa informada não corresponde a esta rota.');
    err.statusCode = 403;
    throw err;
  }
  if (route.completed_at) {
    const err = new Error('Esta rota já foi concluída — não é possível iniciar uma nova perna.');
    err.statusCode = 409;
    throw err;
  }

  // Proteção contra duplicidade: se o celular do motorista reenviar a mesma solicitação (por
  // exemplo, depois de uma demora de conexão em que a resposta se perdeu, ou um novo toque no
  // botão), NÃO cria uma perna repetida — devolve a que já existe pra essa rota+perna. Isso é
  // o que garante que a linha do tempo da rota nunca fique com informação duplicada/infiel.
  const legNumber = body.leg_number || 1;
  const existingLeg = await db.get(
    'SELECT * FROM journeys WHERE route_id = ? AND leg_number = ? ORDER BY id LIMIT 1',
    [body.route_id, legNumber]
  );
  if (existingLeg) return existingLeg;
  const openJourney = await db.get(
    'SELECT * FROM journeys WHERE route_id = ? AND completed_at IS NULL ORDER BY id LIMIT 1',
    [body.route_id]
  );
  if (openJourney) return openJourney;
...
async function createArrival(body) {
  ...
  // Proteção contra duplicidade (mesmo motivo do startJourney acima): se esta chegada já foi
  // registrada antes — por exemplo, o celular reenviou a chegada porque a resposta demorou ou
  // se perdeu por causa de conexão instável — devolve a chegada que já existe, sem criar uma
  // segunda. Checa direto por journey_id (e não mais por journey.completed_at): agora a perna só
  // é concluída quando o descarregamento termina, então no momento da chegada a perna ainda está
  // aberta — se a checagem dependesse de completed_at, um reenvio criaria uma chegada duplicada.
  if (body.journey_id) {
    const journey = await db.get('SELECT * FROM journeys WHERE id = ?', [body.journey_id]);
    if (!journey) {
      const err = new Error('Perna da rota não encontrada.');
      err.statusCode = 404;
      throw err;
    }
    const existingArrival = await db.get(
      'SELECT * FROM arrivals WHERE journey_id = ? ORDER BY id LIMIT 1',
      [body.journey_id]
    );
    if (existingArrival) return existingArrival;
  }
  ...
  // A perna NÃO é concluída aqui, nem a rota avança pra próxima base ainda — isso só acontece
  // quando o descarregamento é finalizado (ver finishUnloading), pra permitir medir o tempo de
  // espera (chegada até início do descarregamento) e o tempo de descarregamento em si.
  return getArrival(id);
}

// Etapa 2: motorista confirma que começou o descarregamento (base aberta ou fechada — em ambos
// os casos calculamos o tempo de espera desde a chegada até este momento). Idempotente: se já
// tiver sido registrado antes (reenvio por causa de conexão instável), não sobrescreve o horário
// original nem recalcula o tempo de espera.
async function startUnloading(id, body) {
  const arrival = await getArrival(id);
  if (!arrival) {
    const err = new Error('Chegada não encontrada.');
    err.statusCode = 404;
    throw err;
  }
  if (arrival.unloading_started_at) return arrival;
  const startedAt = (body && body.unloading_started_at) || new Date().toISOString();
  const waitingMinutes = arrival.arrived_at
    ? Math.round((new Date(startedAt).getTime() - new Date(arrival.arrived_at).getTime()) / 60000)
    : null;
  await db.run(
    'UPDATE arrivals SET unloading_started_at = ?, waiting_minutes = ? WHERE id = ?',
    [startedAt, waitingMinutes, id]
  );
  return getArrival(id);
}

// Etapa 3: motorista confirma que terminou o descarregamento — só AGORA a perna é concluída e a
// rota avança pra próxima base (ou é concluída, se essa era a última). Também idempotente.
async function finishUnloading(id, body) {
  const arrival = await getArrival(id);
  if (!arrival) {
    const err = new Error('Chegada não encontrada.');
    err.statusCode = 404;
  ...
  if (arrival.unloading_completed_at) return arrival;
  if (!arrival.unloading_started_at) {
    const err = new Error('É preciso registrar o início do descarregamento antes de finalizar.');
    err.statusCode = 409;
    throw err;
  }
  const completedAt = (body && body.unloading_completed_at) || new Date().toISOString();
  const unloadingMinutes = Math.round(
    (new Date(completedAt).getTime() - new Date(arrival.unloading_started_at).getTime()) / 60000
  );
  await db.run(
    'UPDATE arrivals SET unloading_completed_at = ?, unloading_minutes = ? WHERE id = ?',
    [completedAt, unloadingMinutes, id]
  );
  if (arrival.journey_id) await completeJourney(arrival.journey_id, id);
  if (arrival.route_id) await advanceRoute(arrival.route_id, Number(arrival.leg_number) || 1, arrival.base);
  return getArrival(id);
}
...
async function handleApi(req, res, url) {
  ...
  try {
    ...
    if (seg[0] === 'arrivals') {
      ...
      if (seg[2] === 'start-unloading' && method === 'PATCH') {
        const body = await readJsonBody(req).catch(() => ({}));
        const r = await startUnloading(Number(seg[1]), body);
        return r ? sendJson(res, 200, r) : sendJson(res, 404, { error: 'not found' });
      }
      if (seg[2] === 'finish-unloading' && method === 'PATCH') {
        const body = await readJsonBody(req).catch(() => ({}));
        const r = await finishUnloading(Number(seg[1]), body);
        return r ? sendJson(res, 200, r) : sendJson(res, 404, { error: 'not found' });
      }
