...
async function getRouteByPlate(plate) {
  // pega a rota mais recente da placa, esteja ela em andamento ou já concluída —
  // assim, ao terminar a última perna, o motorista continua vendo a rota (comprovante)
  // em vez de tomar "nenhuma rota em aberto". Uma rota concluída de outro dia não conta:
  // nesse caso é como se não houvesse rota (o motorista precisa de um lançamento novo).
...
async function createRoute(body) {
  ...
  // uma mesma placa não pode ter duas rotas no mesmo dia — evita duplicidade de informação.
  ...
  if (dup) {
    const err = new Error(
      `A placa ${body.plate} já tem uma rota lançada hoje (rota #${dup.id}). Cada placa só pode ter uma rota por dia.`
    );
    err.statusCode = 409;
    throw err;
  }
